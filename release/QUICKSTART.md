...
CodeDungeon completion is PR-centered and verification-gated: a workflow is not complete unless build/check/test verification is recorded with `codedungeon qa record`, the branch is pushed, a PR exists, adversarial review is generated by `codedungeon review run` from `review-manifest.json` and persona outputs, the review is posted to the PR, and the verdict is `APPROVED`. Review approval does not replace verification. Final reports must come from `codedungeon report render`; hand-written review or final reports are not gate evidence.

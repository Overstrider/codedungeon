...
CodeDungeon completion is PR-centered and verification-gated: there is no local-only completion path, and a workflow reaches `READY_FOR_USER_REVIEW` only after build/check/test verification is executed with `codedungeon qa run`, the branch is pushed, a PR exists and remains open, adversarial review is generated by `codedungeon review run`, and the review is posted with `codedungeon review post` from the latest validated review evidence directory. Review approval does not replace verification. Final reports must come from `codedungeon report render`; hand-written review or final reports are not gate evidence. The user performs final review and merge.

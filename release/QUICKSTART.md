...
CodeDungeon completion is PR-centered and verification-gated: a workflow is not complete unless build/check/test verification passes, the branch is pushed, a PR exists, adversarial review is posted to the PR, and the verdict is `APPROVED`. Review approval does not replace verification. Final output always uses the CodeDungeon PR Report block with PR URL, review verdict, cycles, verification, and next action.

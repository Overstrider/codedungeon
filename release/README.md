...
CodeDungeon completion is PR-centered and verification-gated. Code-writing workflows require a GitHub remote and authenticated `gh`; there is no local-only completion path. A workflow is `COMPLETE` only when build/check/test verification is recorded with `codedungeon qa record`, the branch is pushed, a PR exists, adversarial review is generated by `codedungeon review run` from `review-manifest.json` and persona outputs, the review is posted to the PR, and the final verdict is `APPROVED`. If the same verification command is recorded more than once, the latest record wins. Review approval does not replace verification. Final reports must come from `codedungeon report render`; hand-written review or final reports are not gate evidence.

...
CodeDungeon completion is PR-centered and verification-gated: code-writing workflows are only `COMPLETE` after build/check/test verification is recorded with `codedungeon qa record`, the branch is pushed, a PR exists, adversarial review is generated by `codedungeon review run` from a `review-manifest.json` plus persona outputs, the review is posted to the PR, and the final verdict is `APPROVED`. Review approval does not replace verification. If the same verification command is recorded more than once, the latest record for that command is authoritative. Final reports must be generated with `codedungeon report render`; hand-written review or final reports are not accepted as gate evidence.

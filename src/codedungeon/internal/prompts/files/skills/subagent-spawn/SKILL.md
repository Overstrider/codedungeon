---
name: subagent-spawn
description: "Reference skill: subagent-spawn contract for the Loldinis pipeline. Documents the type matrix (§8.1), spawn prompt template (§8.2), and return contract (§8.3). Other skills and phase commands reference this so isolation rules stay consistent. Does NOT itself spawn subagents — Read-only reference."
---
...
| Architecture plan | `architect-planner` | opus | Phase 1 only. |
...
| QA plan / refine | `qa-planner` | sonnet | Phases 3.5, 5.5. |
| Task decomposition (dev or test) | `task-architect` MODE=dev or test | opus | Phases 4 and 5.6. |
| Project startup | `project-startup` | sonnet | On-demand. |
| Test execution (api / frontend / mobile) | `test-api` or `test-frontend` or `test-mobile` | sonnet | Phase 6. |
...
> Subagent spawning: follow `subagent-spawn/SKILL.md` — type matrix, spawn template, return contract, ROI hook.

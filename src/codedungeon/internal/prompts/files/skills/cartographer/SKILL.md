...
uv run ${CLAUDE_PLUGIN_ROOT}$HOME/.claude/skills/personal/cartographer/scripts/scan-codebase.py . --format json

# Option 2: Direct execution (requires tiktoken installed)
${CLAUDE_PLUGIN_ROOT}$HOME/.claude/skills/personal/cartographer/scripts/scan-codebase.py . --format json

# Option 3: Explicit python (detect command first)
# Detect the correct python command for this OS:
PYTHON_CMD=$(command -v python3 2>/dev/null || command -v python 2>/dev/null || echo "")
if [ -z "$PYTHON_CMD" ]; then echo "ERROR: No python found"; exit 1; fi
$PYTHON_CMD ${CLAUDE_PLUGIN_ROOT}$HOME/.claude/skills/personal/cartographer/scripts/scan-codebase.py . --format json
```
...
**Python not found:**
Detect the correct command: `command -v python3 || command -v python`. On Windows, use `python` (not `python3`). Or use `uv run` which handles Python automatically.

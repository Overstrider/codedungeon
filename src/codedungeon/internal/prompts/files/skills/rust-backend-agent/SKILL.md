...
If this agent itself needs to spawn sub-work (rare), follow `.claude/skills/subagent-spawn/SKILL.md (or $HOME/.claude/plugins/local/codedungeon/skills/subagent-spawn/SKILL.md as fallback)` — explicit type matrix, return contract, ROI hook auto-logged.

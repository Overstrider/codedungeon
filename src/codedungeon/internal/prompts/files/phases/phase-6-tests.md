...
2. Spawn a `general-purpose` agent with **model: `opus`**:
   ...
   (Pass this to test-frontend agents for E2E test writing guidance)

   Follow the codedungeon-test-loop protocol exactly:
   - Step 0: Validate input
   - Step 1: Start the project (project-startup agent)
...
Thinking budget inherited from `PHASE_THINKING[6]` in the orchestrator (`codedungeon-dev-cycle.md`). Model tier via `codedungeon config model <reasoning|fast>` (Sprint 7).

...
**Goal**: Spawn `task-architect` in MODE=dev to decompose enriched {repo}plan.md files into canonical task files per `.claude/tasks/TEMPLATE.md` (200–500 tokens each) + MASTER.md + PLAN.md per repo.
...
You are the task-architect operating in MODE=dev.
...
**Test Auth Injection (Step 4.1 continued):** If `TEST_AUTH_MISSING_REPOS` is not empty, append this block to the task-architect prompt above, right before `CRITICAL: FULLY AUTONOMOUS`:
...
Thinking budget inherited from `PHASE_THINKING[4]` in the orchestrator (`codedungeon-dev-cycle.md`). Model tier via `codedungeon config model <reasoning|fast>` (Sprint 7).

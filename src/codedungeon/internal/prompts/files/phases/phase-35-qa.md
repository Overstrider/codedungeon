...
**Goal**: Spawn the `qa-planner` agent for each affected repo to produce QA test strategy documents.
...
3. Spawn a `general-purpose` agent with **model: `opus`**:
   ...
   ```
   You are the qa-planner for this project.
   ...
   5. Include EXACT curl commands for API tests (the test-api agent executes them literally)
   ...
   **Test Auth Injection (Step 3.5.1 continued):** For each repo that is in `TEST_AUTH_MISSING_REPOS`, you MUST append the following block to that repo's qa-planner prompt above, right before `YOUR JOB:`:

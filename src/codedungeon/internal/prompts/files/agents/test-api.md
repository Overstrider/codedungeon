---
name: test-api
description: "API test executor. Delegates curl + response validation to `codedungeon qa validate-api`. Agent reads the qa-planner's test task, builds the step JSON, invokes the CLI, and creates fix tasks on failure. Validation is ephemeral — no persistent test files."
...
Execute API test steps from a qa-planner task. Curl + status/body/shape/error-quality validation delegated to `codedungeon qa validate-api` (Go binary, deterministic). Agent orchestrates: read task, convert to step JSON, invoke CLI, interpret results, create fix tasks.
...
- `verdict: FAIL` → categorize via `.checks[] | select(.pass==false)`:
  ...
  - Curl command malformed (qa-planner authored wrong spec) → create `test-fix-NN.md`.
...
- Writing Playwright specs (use `test-frontend` agent).
- Writing unit/integration tests (those are LLM-authored code, different task kind).
- Designing test scenarios (qa-planner owns that).

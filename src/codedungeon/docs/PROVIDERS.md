...
The pack controls install paths through `prompts.ArtifactsFor(providerName)`. Codex skills intentionally install under `.agents/skills`, while Codex agents install under `.codex/agents`. Claude workflow skills install under `.claude/skills`, with thin `.claude/commands` wrappers for slash-command discovery. Codex config enables `multi_agent_v2` project-locally through `.codex/config.toml`; runtime Codex launches that need the agent system pass `--enable multi_agent_v2` on the command line instead of mutating user-global feature flags. Do not set `agents.max_threads` or `agents.max_depth` in the project config while this flag is enabled; current Codex builds reject that combination and the Codex defaults are already 6 and 1. Editable command playbooks and phase prompts install under `.codedungeon/commands` and `.codedungeon/phases` for all providers, so provider wrappers and skills can stay native without storing editable playbook content in provider directories.

...
func renderCodedungeonSection() string {
	...
	if p.Name() == "codex" {
		...
		b.WriteString("Promoted workflow surface: `$codedungeon [--full|--lite|--oneshot|--auto] <prompt>`. Without a flag, `$codedungeon` selects automatically and prints `CODEDUNGEON_MODE_SELECTED: <mode> - <reason>` before dispatch.\n\n")
	...
	b.WriteString("| `/code-review` | Standalone adversarial review on current branch. |\n")
	b.WriteString("\nWithout a flag, `/codedungeon` selects automatically and prints `CODEDUNGEON_MODE_SELECTED: <mode> - <reason>` before dispatch. Compatibility aliases remain installed: `/one-shot`, `/side-quest`, and `/main-quest`.\n")

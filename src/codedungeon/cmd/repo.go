...
func renderCodedungeonSection() string {
	...
	if p.Name() == "codex" {
		...
		b.WriteString("|----------|----------|\n")
		b.WriteString("| `one-shot` | Small tasks: plan, code, PR, review; no task split. |\n")
		b.WriteString("| `side-quest` | Simple tasks, single-repo. |\n")
		b.WriteString("| `main-quest` | Complex features, multi-repo, full phase pipeline. |\n")
	...
	b.WriteString("|---------|----------|\n")
	b.WriteString("| `/one-shot` | Small tasks: plan, code, PR, review; no task split. |\n")
	b.WriteString("| `/side-quest` | Simple tasks, single-repo. Plan in plan mode first, then `/side-quest` splits, executes, reviews, creates PR. |\n")
	b.WriteString("| `/main-quest` | Complex features, multi-repo. Full 10-phase pipeline with architect, QA, tests, formal report. |\n")
	b.WriteString("| `/code-review` | Standalone adversarial review on current branch. |\n")
